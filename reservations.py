...
class ReservationManager:
    ...
    def __init__(self, slots_manager: SlotsManager, policy_manager: PolicyManager):
    ...
    def make_reservation(self, service_name, start_time, user, force_past_slots=False, force_advance_cancelation=False, force_default_grid=True, minutes_duration=None):
        ...
        if start_time.minute % self.slots_manager.slot_minutes_duration:
        ...
        if not force_advance_cancelation and start_time<curr_time + timedelta(minutes=self.policy_manager.min_advance_booking_minutes):
        ...
        slots_to_book = self.slots_manager.get_slots(start_time=start_time, end_time=end_time, same_segment_only=True)
        if not slots_to_book:
            raise ClosingTimeError('Out of working hours')
        try:
            self.slots_manager.lock_slots(slots_to_book)
            self.slots_manager.reserve_slots(slots=slots_to_book)
        except Exception as e:
            self.slots_manager.unlock_slots(slots_to_book)
            raise e
    ...
    def cancel_reservation(self, reservation_id, user, force_past_slots=False, force_advance_cancelation=False):
        ...
        slots_to_unbook = self.slots_manager.get_slots(start_time=booked_start_time, end_time=booked_end_time, same_segment_only=True)
        try:
            self.slots_manager.lock_slots(slots_to_unbook)
            self.slots_manager.free_slots(slots_to_unbook)
        except Exception as e: 
            print(e) ##should never happen as far as a whole segment is deleted or a cancelation is done not-properly
            self.slots_manager.unlock_slots(slots_to_unbook)
    ...
    def update_reservation(self, old_reservation_id, user, new_start_time, new_service_name=None, new_minutes_duration=None, force_past_slots=False, force_advance_cancelation=False):
        ...
        update_constraints_params = {'min_reserve_minutes': self.policy_manager.min_advance_cancelation_minutes,
                                     'min_cancel_minutes': self.policy_manager.min_advance_booking_minutes}
        if force_past_slots:
        ...
        slots_to_book = self.slots_manager.get_slots(start_time=new_start_time, end_time=new_end_time, same_segment_only=True)
        slots_to_free = self.slots_manager.get_slots(start_time=old_start_time, end_time=old_end_time, same_segment_only=True)
        ...
        try:
            ...
            Unlock slots.
            """
            self.slots_manager.lock_slots(slots_to_free)
            self.slots_manager.lock_slots(slots_to_book)
            self.slots_manager.free_slots(slots_to_free)
            self.slots_manager.reserve_slots(slots_to_book)
            ...
            self.slots_manager.unlock_slots(slots_to_free)
            self.slots_manager.unlock_slots(slots_to_book)
            return (True, new_reservation_id, old_service, old_start_time)
        except Exception as e:
            #self.slots_manager.free_slots(slots_to_book)
            self.slots_manager.reserve_slots(slots_to_free)
            print(slots_to_free, slots_to_book)
    ...
    def get_available_slots(self, start_time, end_time, service_name, minutes_duration=None, force_past_slots=True):
        ...
        if not force_past_slots and start_time<validate_time(datetime.datetime.now()):#+timedelta(minutes=self.policy_manager.min_advance_booking_minutes)):
        ...
        if minutes_duration is None:
            minutes_duration = self.policy_manager.services[service_name].minutes_duration
        return self.slots_manager.get_available_booking_slots(min_start_time=start_time, max_start_time=end_time, minutes_duration=minutes_duration, 
